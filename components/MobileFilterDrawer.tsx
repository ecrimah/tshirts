    ...
    <div className="fixed inset-0 z-50 lg:hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-2xl max-h-[90vh] flex flex-col">
        <div className="sticky top-0 bg-white border-b border-gray-200 px-4 py-4 rounded-t-2xl">
          ...
          {getActiveFiltersCount() > 0 && (
            <div className="flex items-center justify-between mt-3">
              ...
              <button
                onClick={clearAll}
                className="text-sm text-store-ink font-medium whitespace-nowrap"
              >
                Clear All
              </button>
            </div>
              ...
              {categories.map((category) => (
                <label key={category} className="flex items-center space-x-3 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 text-store-ink border-gray-300 rounded focus:ring-store-primary"
                  />
                ...
                <button
                  ...
                  className={`px-4 py-2 text-sm rounded-lg border transition-colors whitespace-nowrap ${
                    selectedSizes.includes(size)
                      ...
                      : 'bg-white text-gray-700 border-gray-300 hover:border-store-navy'
                  }`}
                >
                  {size}
                </button>
              ...
              {[4, 3, 2, 1].map((rating) => (
                <label key={rating} className="flex items-center space-x-3 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 text-store-ink border-gray-300 focus:ring-store-primary"
                  />
        ...
        <div className="sticky bottom-0 bg-white border-t border-gray-200 p-4">
          <button
            onClick={onClose}
            className="w-full bg-store-navy text-white py-3 rounded-lg font-medium hover:bg-store-navy-light transition-colors whitespace-nowrap"
          >
            Show Results ({234})
          </button>
        </div>
