  ...
  if (error || !order) {
    return (
      <main className="min-h-screen flex flex-col items-center justify-center px-4 text-center">
        ...
        <Link href="/account?tab=orders" className="text-store-primary font-semibold">
          Back to orders
        </Link>
      </main>
    );
      ...
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-8 print:hidden">
          ...
          <button
            ...
            className="px-4 py-2 bg-store-navy text-white rounded-lg font-semibold"
          >
            Print / Save PDF
          </button>
        </div>
        ...
        <div className="border border-gray-200 rounded-xl p-8 print:border-0 print:p-0">
          <div className="flex items-start justify-between gap-4 mb-8">
            <div>
              <p className="text-2xl font-bold tracking-wide">MAMATOR</p>
              <p className="text-sm text-gray-600">Trading Enterprise</p>
            </div>
            ...
            <tbody>
              {items.map((item: any) => (
                <tr key={item.id} className="border-b border-gray-100">
                  ...
                  <td className="py-3 pr-2">GH₵{Number(item.unit_price || 0).toFixed(2)}</td>
                  <td className="py-3 text-right">
                    GH₵{Number(item.total_price ?? Number(item.unit_price || 0) * item.quantity).toFixed(2)}
                  </td>
                </tr>
              ))}
            </tbody>
