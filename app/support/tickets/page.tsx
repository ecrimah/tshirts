...
export default function MyTicketsPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'open': return 'bg-store-primary/15 text-store-primary';
      case 'in_progress': return 'bg-amber-100 text-amber-700';
      case 'resolved': return 'bg-store-primary/15 text-store-primary';
  ...
  const getPriorityColor = (priority: string) => {
    switch (priority) {
      ...
      case 'normal': return 'text-store-primary';
      ...
      <main className="min-h-screen bg-gray-50 py-12">
        <div className="max-w-7xl mx-auto px-4">
          <div className="flex items-center justify-between mb-8">
            ...
            <Link
              href="/support/ticket"
              className="bg-store-navy hover:bg-store-navy text-white px-6 py-3 rounded-lg font-semibold transition-colors whitespace-nowrap"
            >
            ...
            <div className="flex border-b border-gray-200 overflow-x-auto">
              <button
                ...
                className={`px-6 py-4 font-semibold whitespace-nowrap border-b-2 transition-colors ${
                  selectedFilter === 'all'
                    ? 'border-store-navy text-store-primary'
          ...
          {filteredTickets.length === 0 ? (
            <div className="bg-white rounded-xl shadow-sm p-12 text-center">
              ...
              <Link
                href="/support/ticket"
                className="inline-block bg-store-navy hover:bg-store-navy text-white px-6 py-3 rounded-lg font-semibold transition-colors whitespace-nowrap"
              >
            ...
            <div className="space-y-4">
              {filteredTickets.map((ticket) => (
                <Link
                  ...
                  className="block bg-white rounded-xl shadow-sm p-6 hover:shadow-lg transition-all border-2 border-transparent hover:border-store-navy"
                >
          ...
          <div className="mt-8 grid md:grid-cols-3 gap-6">
            <div className="bg-white rounded-xl shadow-sm p-6">
              <div className="w-12 h-12 flex items-center justify-center bg-store-primary/15 rounded-lg mb-4">
                <i className="ri-time-line text-2xl text-store-primary"></i>
              </div>
              <h3 className="font-bold text-gray-900 mb-2">Response Time</h3>
              <p className="text-2xl font-bold text-store-primary mb-1">24 hours</p>
              <p className="text-sm text-gray-600">Average response time</p>
            </div>

            <div className="bg-white rounded-xl shadow-sm p-6">
              <div className="w-12 h-12 flex items-center justify-center bg-store-primary/15 rounded-lg mb-4">
                <i className="ri-checkbox-circle-line text-2xl text-store-primary"></i>
              </div>
              <h3 className="font-bold text-gray-900 mb-2">Resolution Rate</h3>
              <p className="text-2xl font-bold text-store-primary mb-1">95%</p>
