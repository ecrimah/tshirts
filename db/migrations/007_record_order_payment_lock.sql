-- Harden record_order_payment: row lock + consistent return after concurrent updates.
...
BEGIN
  ...
  SET
    ...
    metadata = COALESCE(metadata, '{}'::jsonb) ||
               jsonb_build_object(
                   'moolre_reference', moolre_ref,
                   'deposit_verified_at', to_char(now() AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'),
...
GRANT EXECUTE ON FUNCTION public.record_order_payment(text, text, numeric) TO store_mamator;
